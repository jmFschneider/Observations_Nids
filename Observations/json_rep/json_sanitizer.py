...
def validate_json_structure(data):
    ...
    required_top_keys = [
        ...
        "tableau_donnees", "tableau_donnees_2", "causes_echec"
    ]
    ...
    if "localisation" in data:
        expected = {
            "IGN_50000", "commune", "dep_t",
            "coordonnees_et_ou_lieu_dit", "altitude", "paysage", "alentours"
        }
        missing = expected - set(data["localisation"].keys())
        if missing:
            errors.append(f"localisation : champs manquants : {missing}")

    if "tableau_donnees" in data:
        if not isinstance(data["tableau_donnees"], list):
            errors.append("tableau_donnees doit être une liste")

    if "tableau_donnees_2" in data:
        expected = {
            "1er_o_pondu", "1er_p_eclos", "1er_p_volant",
    ...
    if "causes_echec" in data:
        if "causes_d_echec" not in data["causes_echec"]:
    ...
    def ensure_keys(dct, keys):
        for k in keys:
            if k not in dct or not isinstance(dct[k], dict):
                dct[k] = {"jour": None, "Mois": None, "Precision": None} if "1er" in k else {
                    "pondus": None, "eclos": None, "n_ecl": None} if k == "nombre_oeufs" else {
                    "1/2": None, "3/4": None, "vol_t": None}

    if "tableau_donnees_2" in corrected:
        td2 = corrected["tableau_donnees_2"]
        ensure_keys(td2, ["1er_o_pondu", "1er_p_eclos", "1er_p_volant", "nombre_oeufs", "nombre_poussins"])

        # Corriger cas particuliers où ces champs sont des listes (à plat)
        for key in ["1er_o_pondu", "1er_p_eclos", "1er_p_volant", "nombre_oeufs", "nombre_poussins"]:
            if isinstance(td2.get(key), list) and len(td2[key]) > 0:
                td2[key] = td2[key][0]

    # Corriger champ 'causes_echec'
    if "causes_echec" in corrected:
        if "causes d'échec" in corrected["causes_echec"]:
            corrected["causes_echec"]["causes_d_echec"] = corrected["causes_echec"].pop("causes d'échec")
