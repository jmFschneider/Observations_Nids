...
3. Génération du token
   ├─> Token sécurisé Django (default_token_generator)
...
**Vue de demande** : `accounts/views/auth.py::mot_de_passe_oublie()`
- URL : `/accounts/mot-de-passe-oublie/`
- Méthode : GET (affichage) / POST (traitement)
- Template : `accounts/templates/accounts/mot_de_passe_oublie.html`

**Vue de réinitialisation** : `accounts/views/auth.py::reinitialiser_mot_de_passe()`
- URL : `/accounts/reinitialiser-mot-de-passe/<uidb64>/<token>/`
- Méthode : GET (affichage) / POST (enregistrement)
- Template : `accounts/templates/accounts/reinitialiser_mot_de_passe.html`

**Service email** : `accounts/utils/email_service.py::envoyer_email_reinitialisation_mdp()`
- Template email : `accounts/templates/accounts/emails/reinitialisation_mot_de_passe.html`
- Envoi HTML + texte
- Lien cliquable + lien copié/collable

### Sécurité

✅ **Protection contre l'énumération d'emails**
- Message identique que l'email existe ou non
- Logs séparés pour le monitoring

✅ **Token sécurisé**
- Basé sur le hash du mot de passe actuel
- Invalide automatiquement après changement
- Expiration après 24h (configurable via PASSWORD_RESET_TIMEOUT)
...
- La réinitialisation ne fonctionne que pour is_active=True

### Gestion des emails multiples

Si plusieurs utilisateurs ont le même email (situation anormale mais gérée) :
- Un email est envoyé à chaque utilisateur
- Chaque lien est unique et valide pour son utilisateur
- Logs indiquent le nombre de comptes concernés

**Prévention** : Contrainte d'unicité sur le champ email (voir section suivante)
...
- [Réinitialisation de mot de passe](./REINITIALISATION_MOT_DE_PASSE.md)
- [Déploiement](../deployment/CELERY_DEPLOYMENT.md)
