...
Ce guide vous accompagne pour déployer l'application **Observations Nids** sur un Raspberry Pi ou un serveur Linux en production. Le processus se déroule en 3 étapes principales :
...
- ✅ Raspberry Pi 3B+ ou supérieur (recommandé : Pi 4 avec 4GB RAM) ou serveur Linux Debian/Ubuntu
- ✅ Carte SD de qualité (32GB minimum, classe A1 ou A2)
- ✅ Raspberry Pi OS 64-bit (Debian Bookworm) ou Ubuntu Server 22.04+ à jour
...
- ✅ Serveur web Apache2 ou Nginx installé
- ✅ Redis, MariaDB ou MySQL
...
### Méthée 1 : Déploiement automatisé (recommandé)
...
Redis devrait déjà être installé (voir Prérequis). Vérification :
...
## 5. Déploiement Django avec Gunicorn (Alternative à mod_wsgi)
...
## 6. Configuration du Serveur Web (Apache ou Nginx)

### Option A : Apache avec mod_wsgi (Recommandé pour Raspberry Pi)
...
**Note** : Le fichier `observations_nids.wsgi` doit être créé manuellement si vous n'utilisez pas le script `deploy_pi.sh`.
...
### ☑ 3.3 Vérifier les logs Apache/Nginx

```bash
sudo tail -f /var/log/apache2/observations_nids_error.log # Pour Apache
# OU
sudo tail -f /var/log/nginx/observations_nids_error.log # Pour Nginx
```
...
sudo systemctl status celery-worker
sudo systemctl status celery-beat # si activé
sudo systemctl status gunicorn # si utilisé
...
# 9. Redémarrer les services web et Celery
sudo systemctl restart apache2 # ou nginx
sudo systemctl restart gunicorn # si utilisé
sudo systemctl restart celery-worker
sudo systemctl restart celery-beat # si utilisé

# 10. Vérifier les logs
sudo tail -f /var/log/apache2/observations_nids_error.log # ou nginx logs
```
...
sudo certbot --apache -d votre-domaine.fr -d www.votre-domaine.fr # Pour Apache
# OU
sudo certbot --nginx -d votre-domaine.fr -d www.votre-domaine.fr # Pour Nginx
...
#### ☑ 2.2 Hardening Apache/Nginx

**Option A : Hardening Apache**
...
<VirtualHost *:443>
    ...
    Header always set Strict-Transport-Transport-Security "max-age=63072000; includeSubDomains; preload"
...
**Option B : Hardening Nginx**

Les en-têtes de sécurité sont déjà inclus dans la configuration Nginx de base (voir section 6.1 Option B).
...
**A. Configurer la rotation des logs Apache/Nginx**

Créer `/etc/logrotate.d/observations_nids` :

```
/var/log/apache2/observations_nids*.log { # Pour Apache
# OU
/var/log/nginx/observations_nids*.log { # Pour Nginx
    daily
    rotate 14
    compress
    delaycompress
    ...
    sharedscripts
    postrotate
        systemctl reload apache2 > /dev/null 2>&1 || true # Pour Apache
        # OU
        systemctl reload nginx > /dev/null 2>&1 || true # Pour Nginx
    endscript
}
```
...
DB_NAME="observations_nids"
DB_USER="django_app" # ou obs_user si utilisé
...
- [ ] Vérifier les logs d'erreur Apache/Nginx : `sudo tail -n 100 /var/log/apache2/observations_nids_error.log` (ou Nginx logs)
...
- [ ] Analyser les logs d'accès Apache/Nginx pour détecter des comportements suspects
...
- [ ] Apache/Nginx configuré (VirtualHost + mod_wsgi/Gunicorn)
...
- [ ] Logs Apache/Nginx sans erreurs
...
systemctl is-active apache2 redis-server mariadb fail2ban ufw # ou nginx
...
echo -e "\n=== Logs Apache/Nginx (dernières erreurs) ==="
tail -n 5 /var/log/apache2/observations_nids_error.log # ou Nginx logs
```
...
#### Apache/Nginx ne démarre pas

```bash
# Vérifier la configuration
sudo apachectl configtest # ou nginx -t

# Vérifier les logs
sudo tail -n 50 /var/log/apache2/error.log # ou nginx logs
...
sudo tail -n 100 /var/log/apache2/observations_nids_error.log # ou Nginx logs
