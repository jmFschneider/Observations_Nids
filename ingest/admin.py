...
class PreparationImageAdmin(admin.ModelAdmin):
    list_display = ['id', 'date_preparation', 'operateur', 'fichier_brut_recto', 'fichier_brut_verso']
    ...
    fieldsets = (
        ('Fichiers sources', {
            'fields': ('fichier_brut_recto', 'fichier_brut_verso')
        }),
        ('Fichier résultat', {
            'fields': ('fichier_fusionne',)
        }),
        ('Métadonnées de traitement', {
            'fields': ('operations_effectuees', 'notes')
        }),
        ('Traçabilité', {
            'fields': ('operateur', 'date_preparation')
        }),
    )
