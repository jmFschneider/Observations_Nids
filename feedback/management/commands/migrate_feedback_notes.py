...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not commit:
            self.stdout.write(
                self.style.WARNING(
                    "MODE DRY-RUN : Aucun changement ne sera appliqué. Utilisez --commit pour valider."
                )
            )

        feedbacks_with_notes = Feedback.objects.exclude(admin_note="")
        self.stdout.write(
            f"Trouvé {feedbacks_with_notes.count()} feedbacks avec des notes à migrer."
        )
        ...
        with transaction.atomic():
            for feedback in feedbacks_with_notes:
                ...
                if commit:
                    ...
                    self.stdout.write(
                        f"  [Simulé] Création d'un message : {feedback.admin_note[:50]}..."
                    )
        ...
        if commit:
            ...
            self.stdout.write(
                self.style.WARNING("Fin de la simulation. Relancez avec --commit pour appliquer.")
            )
